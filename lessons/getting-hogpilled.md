I am a potential user of PostHog. 
...
Next, walk me through each bite-size concept, in order, one step at a time. Ask questions as we go to check that I have understood each concept. 
...
Remember that PostHog Code might be configured in a lot of different ways visually and is constantly evolving, so avoid assumptions about where a UI element might be. The file explorer may be on the left or the right.
...
(If I already have a product I’m working on) At the end of any lessons, you should then prompt me to use the Wizard to set up PostHog, customizing the Wizard in line with the information you have learned about me and my product. Use the command: `npx @posthog/wizard@latest`
